...
- **Cadastro pede só usuário+senha; login usa `username#discriminator`+senha, não email** (FASE 11 — pedido explícito do usuário do projeto). `email` continua existindo no schema (`null: true`) para uma fatia futura de confirmação/recuperação de senha, mas o cliente nunca pede ou guarda isso hoje. O dia a dia não passa pela tela de login: a sessão persiste via `safeStorage` (ver abaixo); login só é necessário se essa sessão local se perder — por isso o formulário pede o identificador completo com discriminator, não só o username (que sozinho não é único).
