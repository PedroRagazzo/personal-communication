...
- **Media Plane**: real audio/video/screen-share bytes, via WebRTC (DTLS-SRTP) — **never** touches the Phoenix WebSocket. Phoenix Channels only relay SDP offers/answers and ICE candidates as opaque JSON (or, for Go Live, just hand out a LiveKit token — no SDP/ICE relay at all, since the SFU handles that). Client-side peer connections are real for voice (audio), video (camera), and screen share — all three share the same mesh (`desktop/src/renderer/src/webrtc/MeshManager.ts` — see "Desktop client" below); the actual LiveKit connection is still a server-only skeleton.
