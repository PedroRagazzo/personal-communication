...
TORA DOS BURRO is a Discord-style real-time communication platform: servers/channels, roles/permissions, real-time chat, voice/video/screen-share over WebRTC, and Go Live (via LiveKit). Backend (Elixir/Phoenix): MVP (FASES 0-8) plus FASE 9 (Go Live) complete. Desktop client (React + Electron, `desktop/`): **FASE 11 is complete** — working auth, server/channel navigation (including joining an existing server via invite code), real-time chat, voice (real mesh WebRTC), screen share, video/camera, Go Live (LiveKit, client-side wired up — publish/subscribe of real media through an actual LiveKit server is still unverified, since this environment has no Docker), and a real Windows installer (`electron-builder`, NSIS, unsigned, no auto-update). The actual single base server for this deploy ("TORA DOS BURRO") already exists, owned by the real account `Ragazzo#9054`. Rust/C++/Python media services are planned for later and don't exist yet beyond empty placeholder directories (`rust/`, `cpp/`, `python/`).
